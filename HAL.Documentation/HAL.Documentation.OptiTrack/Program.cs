...
namespace HAL.Documentation.OptiTrack
{
    class Program
    {
        ...
        private static void OnManagerStateChanged(Objects.IState current, Objects.IState previous)
        {
            ...
            {
                // Console display of the name of the detected bodies and the number of marker they contain 
                ...
                // Bodie's frames in the Reference world
                var bodiesOrientedPositions = state.Bodies.Select(bf => bf.GetReferencedPosition(OptiTrack.Reference).LocationInWorld(true));
            }
        }
    }
}
